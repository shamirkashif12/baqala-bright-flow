...
function RtsSheet({ open, onOpenChange, onCreated }: {
  ...
  // Manual supplier override when WR lookup has no supplier ID
  ...
  const lookupOrder = async () => {
    ...
    try {
      ...
      // ── Second fallback: supplier_to_warehouse stock transfer (PO-xxx IDs) ──
      try {
        const t = await api.getStockTransferByNumber(num);
        if (t && t.transferType === "supplier_to_warehouse") {
          setFetchedSupplierId(t.sourceSupplierId ?? "");
  ...
  const effectiveSupplierId = fetchedSupplierId || manualSupplierId;
      ...
      <SheetContent className="w-[540px] overflow-y-auto">
        ...
        {step === 2 && (
          <div className="mt-5 space-y-5">
            ...
            {/* ── Supplier — shown only when WR lookup didn't resolve one ── */}
            {!fetchedSupplierId && (
              <div className="space-y-1.5">
                <Label className="text-xs font-medium">Return to Supplier *</Label>
              ...
              <tbody>
                {displayRows.map(({ key, group, isBatch }) => {
                  ...
                  const creditValue = group.reduce(
                    (s, tr) => s + (tr.items ?? []).reduce(
                      (si, i) => si + (i.receivedQuantity ?? i.requestedQuantity) * (i.unitCost ?? 0), 0
                    ), 0
                  );
                  ...
                  return (
                    <tr key={key} className="border-b border-border/40 hover:bg-muted/30 last:border-0">
                      ...
                      <td className="px-4 py-3 text-xs text-muted-foreground">{allWarehouses}</td>

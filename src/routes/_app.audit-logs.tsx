...
import {
  ...
  Settings as SettingsIcon, CreditCard, BadgePercent, ShoppingCart, RefreshCw, type LucideIcon,
} from "lucide-react";
                ...
                <li key={l.id} className="flex items-start gap-4 px-4 py-3.5 hover:bg-muted/30 transition-colors">
                  ...
                  <div className="flex-1 min-w-0 space-y-0.5">
                    ...
                    <div className="flex flex-wrap items-center gap-2">
                      <p className="font-semibold text-sm leading-tight">{l.action}</p>
                    ...
                    {/* Reference · details */}
                    {(l.entityId || l.newValues || l.details) && (
                      <p className="text-[11px] text-muted-foreground font-mono">
                        {l.entityId && <span className="text-foreground/80">{l.entityId.slice(0,8)}…</span>}
                        {l.entityId && (l.newValues || l.details) && <span> · </span>}
                        {(l.newValues || l.details) && <span>{l.newValues ?? l.details}</span>}
                      </p>
                    )}
                    {/* by User · time */}
